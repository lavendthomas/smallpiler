...
void c(node *x) {
    switch (x->kind) {
        ...
        case WHILE : {
            ...
            // Go through all the node < here and if there is a GOTO 0 then we have to change it to the end of the while
            code *a = x->start;
            code *b = a + 1;
            while (b <= here) { // Go through all the bytecode generated by the while
                if (*a == GOTO && *b == 0) {
                    // There is GOTO 0, which is a placeholder for a break, so we replace the GOTO 0 by a
