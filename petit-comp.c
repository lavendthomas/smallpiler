...
struct node
  {
    int kind;
    long start;            // Number of bytes since beginning of the bytecode of this operation
    struct node *parent;
    struct node *o1;
    struct node *o2;
    struct node *o3;
...
const int MAX_BYTECODE_SIZE = 1000;     // Initial size of the bytecode
    ...
    switch (x->kind) {
        ...
        case CONTINUE : {
            ...
            while (n != NULL && n->kind != PROG) {
                if (n->kind == WHILE || n->kind == DO) {
                    // We found the nearest while to the continue
                    fix(here++, n->start + object);            // Go back to the start of the loop
                    break;
                }
                n = n->parent;
            }
